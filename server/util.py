...
def load_saved_artifacts():
    ...
    # Get the absolute path of the current script (util.py)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    class_dict_path = os.path.join(current_dir, "class_dictionary.json")
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    # Get OpenCV's built-in Haarcascade path
    ...
    if face_cascade.empty() or eye_cascade.empty():
        raise Exception("Error loading Haarcascade files. Ensure OpenCV is installed correctly.")
...
if __name__ == '__main__':
    ...
    result = classify_image(None, image_path)
